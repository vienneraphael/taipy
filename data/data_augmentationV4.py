...
data = pd.read_csv('modified_supermarkt_sales_plus_three_years.csv')
...
new_products = np.random.choice(list(data['Product_line'].unique()), size=num_entries)
...
new_unit_prices = np.random.uniform(40, 200, size=num_entries)
new_quantities = np.random.randint(15, 30, size=num_entries)
new_tax = new_unit_prices * new_quantities * 1.1
...
new_ratings = np.random.uniform(10, 25, size=num_entries)
new_dates = np.random.choice(pd.date_range(start='2024-01-01', end='2024-12-31').date, size=num_entries)
new_times = [f"{np.random.randint(10, 19)}:{np.random.randint(0, 59):02d}" for _ in range(num_entries)]
...
new_cities = np.random.choice(list(cities_latitudes.keys()), size=num_entries, replace = True)
...
additional_data['Date'] = pd.to_datetime(additional_data['Date']).dt.date
...
augmented_data.to_csv('modified_supermarkt_sales_plus_four_years.csv')